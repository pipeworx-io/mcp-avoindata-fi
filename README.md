...
| `query_resource` | Pull rows from a tabular Finland Open Data resource (CKAN DataStore) by resource_id (from a dataset's resources). Optional full-text q, limit, offset. Note: only resources with the DataStore enabled are queryable. |
...
`tools/list` at `https://gateway.pipeworx.io/avoindata-fi/mcp` returns the tools in the table
...
```
ask_pipeworx({ question: "your question about Avoindata Fi data" })
```
